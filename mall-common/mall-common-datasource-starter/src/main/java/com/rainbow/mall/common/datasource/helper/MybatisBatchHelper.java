package com.rainbow.mall.common.datasource.helper;
...
public class MybatisBatchHelper {
    ...
    public <T,V> int batchInsertOrUpdate(List<T> recordList,Class<V> mapper, BiConsumer<V,T> biConsumer){
        ...
        try {
            ...
            // 数据批量处理
            List<List<T>> partition = Lists.partition(recordList, BATCH_SIZE);
            for (List<T> partitionRecordList : partition) {
                for (T record : partitionRecordList) {
                     // 执行biConsumer方法
